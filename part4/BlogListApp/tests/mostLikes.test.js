...
describe("Most Likes", () => {
  test("in an empty list returns an empty object", () => {
    const blogs = [];
  ...
  test("in a single item list returns an that item itself", () => {
    const blogs = [
      {
    ...
    const result = mostLikes(blogs);
    expect(result).toEqual({ author: "Muhammad Tameem Shahid", likes: 32 });
  });

  test("in a multiple item list is calculated right", () => {
    const blogs = [
      {
    ...
    const result = mostLikes(blogs);
    expect(result).toEqual({ author: "Ada Lovelace", likes: 176 });
  });
});
